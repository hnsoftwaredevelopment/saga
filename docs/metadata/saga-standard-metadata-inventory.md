...
| Publication date | `pubdate`, `date` | Supported internally | EPUB/KEPUB OPF, Calibre `metadata.opf`, Saga `metadata.json` | Improve | Details, search, sort later, duplicate merge | Yes | Date parsing must remain conservative. Search matches stable date forms such as year and ISO date. |
